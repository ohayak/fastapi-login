...
class CRUDBatteryCell(CRUDBase[BatteryCell, Dict[str, Any], IBatteryCellRead]):
    async def get_cell_by_name(self, *, name: str, db_session: Optional[AsyncSession]) -> BatteryCell:
...
class CRUDBatteryModel(CRUDBase[BatteryModel, Dict[str, Any], IBatteryModelRead]):
    async def get_cell_by_name(self, *, name: str, db_session: Optional[AsyncSession]) -> BatteryModel:
...
class CRUDBatteryInfo(CRUDBase[BatteryInfo, Dict[str, Any], IBatteryInfoRead]):
    async def get_cell_by_name(self, *, name: str, db_session: Optional[AsyncSession]) -> BatteryInfo:
